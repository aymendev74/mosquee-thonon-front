            ...
            <Row gutter={[16, 32]}>
                <Col span={6}>
                    <Form.Item
                        name="idPeriode"
                        label="Période">
                        <Select options={periodesOptions} onChange={(value) => { setSelectedIdPeriode(value) }} />
                    </Form.Item>
                </Col>
                <Col span={4}>
                    <Tooltip title="Modifier les données de la période sélectionnée" color="geekblue"><Button icon={<EditOutlined />}
                        type="primary" disabled={!selectedIdPeriode}>Modifier</Button></Tooltip>
                    <Tooltip title="Créer une nouvelle période" color="geekblue"><Button icon={<PlusCircleOutlined />} type="primary" className="m-left-10">Créer</Button></Tooltip>
                    <Tooltip title="Consulter/Modifier les tarifs" color="geekblue"><Button icon={<EuroCircleTwoTone />} type="primary" className="m-left-10"
                        disabled={!selectedIdPeriode} onClick={onEditTarif} /></Tooltip>
                </Col>
            </Row>
            {viewTarif && (<InfosTarif />)}
        </Form>
    </>);
}